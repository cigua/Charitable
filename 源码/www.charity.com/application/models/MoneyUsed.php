...
class MoneyUsedModel extends BaseModel {
    public function __construct()
    {
        parent::__construct('charity', 't_money_used', 'mu_id');
    }
    ...
    public function deleteData($id){
        return $this->_db->delete($this->_table)->where('mu_pid', $id)->execute();
    }
    ...
    private function _setWhereSQL ($where = array()) {
        if(isset($where['p_id']) && $where['p_id']){
            $this->_db->where('mu_pid', $where['p_id']);
        }
