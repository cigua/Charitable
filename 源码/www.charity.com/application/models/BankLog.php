...
class BankLogModel extends BaseModel {

    public function __construct()
    {
        parent::__construct('charity', 't_bank_log', 'bl_id');
    }
    ...
    private function _setWhereSQL ($where = array()) {
        ...
        if(A_ROLE > 0){
            $this->_db->where("bl_mid", A_ROLE);
        }

        if (isset($where['type']) AND $where['type']) {
            $this->_db->where("bl_type", $where['type']);
        }

        if (isset($where['username']) AND $where['username']) {
            $this->_db->where("dl_title like '%{$where['username']}%'");
        }
