...
class ArticleModel extends BaseModel {
    ...
    private $_field = 'a_id,c_id,a_img,a_title,a_view,a_addTime,a_order,a_status,a_content';
    public function __construct()
    {
        parent::__construct('charity', 't_article', 'a_id');
    }
    ...
    public function addView($id){
        return $this->_db->update($this->_table)->set('a_view', 1, TRUE)->where($this->_primary_key, $id)->execute();
    }
    ...
    private function _setWhereSQL ($where = array()) {
        if(isset($where['c_id']) && $where['c_id']){
            $this->_db->where('c_id', $where['c_id']);
        }
