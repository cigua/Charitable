...
//            'categoryOption' => $categoryOption,
    ...
    public function formAction($id=0) {
        ...
        if(A_ROLE == 0){
            $companyList = $this->_companyModel->getCompanyOption();
            $companyOption = Helper_Form::option($companyList, intval($row['c_id']), '请选择公司');
        }else{
            $companyList = $this->_companyModel->getCompanyOption();
        ...
        $this->getView()->assign( 'statusOption' , Helper_Form::option($this->_status, intval($row['p_status']), '请选择状态'));
    }
...
//        $upload->setImageMinAttr(150, 150);
    ...
    public function saveAction(){
        ...
        if($id){
            ...
            $msg = '添加';
            $post['p_addtime'] = date('Y-m-d H:i:s');
